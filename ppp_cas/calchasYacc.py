...
from .calchasTree import FunctionCall, List, Id, Number
...
def p_expr_unary(p):
    ...
    if p[1] == '-':
        p[0] = FunctionCall(Id('Mul'), List([Number('-1'), p[2]]))
    else:
        p[0] = p[2]
...
def p_expression_arith_divide(p):
    ...
    p[0] = FunctionCall(Id('Mul'), List([p[1], FunctionCall(Id('Pow'), List([p[3], Number('-1')]))]))


def p_expression_arith_minus(p):
    """expression : expression MINUS expression"""
    p[0] = FunctionCall(Id('Add'), List([p[1], FunctionCall(Id('Mul'), List([p[3], Number('-1')]))]))
...
def p_function_call_arg_diff(p):
    ...
    p[0] = FunctionCall(Id('diff'), List([FunctionCall(Id(p[1]), p[4]), p[4][0], Number(p[2])]))
